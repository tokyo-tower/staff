...
exports.default = (req) => {
    ...
    req.checkBody('lastName', req.__('Message.maxLength{{fieldName}}{{max}}', { fieldName: req.__('Form.FieldName.lastName'), max: NAME_MAX_LENGTH.toString() })).isLength({
        max: NAME_MAX_LENGTH
    });
    req.checkBody('lastName', req.__('Message.invalid{{fieldName}}', { fieldName: req.__('Form.FieldName.lastName') })).matches(/^[ァ-ロワヲンーa-zA-Z]*$/);
    // firstName
    req.checkBody('firstName', req.__('Message.required{{fieldName}}', { fieldName: req.__('Form.FieldName.firstName') })).notEmpty();
    req.checkBody('firstName', req.__('Message.maxLength{{fieldName}}{{max}}', { fieldName: req.__('Form.FieldName.firstName'), max: NAME_MAX_LENGTH.toString() })).isLength({
        max: NAME_MAX_LENGTH
    });
    req.checkBody('firstName', req.__('Message.invalid{{fieldName}}', { fieldName: req.__('Form.FieldName.firstName') })).matches(/^[ァ-ロワヲンーa-zA-Z]*$/);
    // tel
    ...
    req.checkBody('email', req.__('Message.match{{fieldName}}', { fieldName: req.__('Form.FieldName.email') })).equals(`${req.body.emailConfirm}@${req.body.emailConfirmDomain}`);
    // emailConfirm
    req.checkBody('emailConfirm', req.__('Message.required{{fieldName}}', { fieldName: req.__('Form.FieldName.emailConfirm') })).notEmpty();
    // emailConfirmDomain
    req.checkBody('emailConfirmDomain', req.__('Message.required{{fieldName}}', { fieldName: req.__('Form.FieldName.emailConfirmDomain') })).notEmpty();
    // address
